"""
parameter golf submission: int6 STE + NorMuon + NTK rope extrapolation + SWA + sliding window + SmearGate + Muon WD + Seq2048
...
- NorMuon optimizer (row-normalized Newton-Schulz updates) with decoupled weight decay
- NTK-aware RoPE extrapolation
- sliding window eval w/ stride=64
- stochastic weight averaging during warmdown
- optimizer tuning: momentum 0.99, lower LRs, longer warmdown
- SmearGate: blends token embeddings with their predecessors using a learned gate
...
class Hyperparameters:
    ...
    swa_start_frac = float(os.environ.get("SWA_START_FRAC", 0.5))  # start SWA at 50% through warmdown
